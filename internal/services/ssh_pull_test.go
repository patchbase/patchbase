...
func TestCleanQuote(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.input, func(t *testing.T) {
			assert.Equal(t, tc.expected, cleanQuote(tc.input))
		})
	}
}
...
func TestCountAptPackageUpdates(t *testing.T) {
	t.Run("empty", func(t *testing.T) {
		assert.Equal(t, int32(0), countAptPackageUpdates(""))
	})
...
`
		assert.Equal(t, int32(2), countAptPackageUpdates(output))
	})
...
`
		assert.Equal(t, int32(1), countAptPackageUpdates(output))
	})
}

func TestCountRpmPackageUpdates(t *testing.T) {
	t.Run("empty", func(t *testing.T) {
		assert.Equal(t, int32(0), countRpmPackageUpdates(""))
	})
...
`
		assert.Equal(t, int32(2), countRpmPackageUpdates(output))
	})
...
`
		assert.Equal(t, int32(1), countRpmPackageUpdates(output))
	})
}
