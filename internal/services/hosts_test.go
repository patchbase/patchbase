...
func TestHosts_DeleteHost_HandlesPeriodicJobRemovalError(t *testing.T) {
	...
	hostsService := do.MustInvoke[services.Hosts](backend.Injector())
	err = hostsService.DeleteHost(ctx, services.SystemActorRef(), string(host.ID))
	require.NoError(t, err)
...
func TestHosts_RunSSHPull_ProducesMatcherDecisions(t *testing.T) {
	...
	hostsService := do.MustInvoke[services.Hosts](backend.Injector())
	err = hostsService.RunSSHPull(ctx, services.SystemActorRef(), string(hostID))
	require.NoError(t, err)
...
func TestHosts_IngestManualReport(t *testing.T) {
	...
	// Create Manual Host
	hostInfo, err := hostsService.CreateManualHost(ctx, services.SystemActorRef(), "manual-test", "manual-host-test")
	require.NoError(t, err)
...
repo-baseos Enabled`
	...
	err = hostsService.IngestManualReport(ctx, services.SystemActorRef(), hostInfo.ID, []byte(reportContent))
	require.NoError(t, err)
...
func TestHosts_SSHUniqueKeyPairFlagAndGlobalFallback(t *testing.T) {
	...
	resGlobal, err := hostsService.CreateSSHHost(ctx, services.SystemActorRef(), services.CreateSSHHostInput{
	...
	err = hostsService.OnboardSSHHost(ctx, services.SystemActorRef(), resGlobal.HostID)
	require.NoError(t, err)

	err = hostsService.RunSSHPull(ctx, services.SystemActorRef(), resGlobal.HostID)
	require.NoError(t, err)
	assert.Equal(t, globalKey.PrivateKey, mockRunner.calledPrivateKey)

	// 2. Create a host using unique SSH key (UniqueKeyPair = true)
	resUnique, err := hostsService.CreateSSHHost(ctx, services.SystemActorRef(), services.CreateSSHHostInput{
	...
	// Onboard and run SSH pull
	err = hostsService.OnboardSSHHost(ctx, services.SystemActorRef(), resUnique.HostID)
	require.NoError(t, err)

	err = hostsService.RunSSHPull(ctx, services.SystemActorRef(), resUnique.HostID)
	require.NoError(t, err)
...
func TestHosts_UniqueConstraints(t *testing.T) {
	...
	// Create first host
	_, err := hostsService.CreateManualHost(ctx, services.SystemActorRef(), "unique-name-1", "unique-host-1")
	require.NoError(t, err)

	// Try to create second host with same display name
	_, err = hostsService.CreateManualHost(ctx, services.SystemActorRef(), "unique-name-1", "unique-host-2")
	assert.ErrorIs(t, err, apperr.ErrDuplicateHostDisplayName)

	// Create first SSH host
	_, err = hostsService.CreateSSHHost(ctx, services.SystemActorRef(), services.CreateSSHHostInput{
...
func TestHosts_RegisterAgentHost_AuditsAsTokenOwner(t *testing.T) {
	...
	// Seed a registration token owned by u_admin.
	tokenID := id.New("rtok")
	tokenPlain := "pb_reg_testtoken123456789012"
	_, err := queries.InsertRegistrationToken(ctx, db.InsertRegistrationTokenParams{
		ID:              tokenID,
		Name:            "agent-test-token",
		TokenHash:       utils.SHA256(tokenPlain),
		CreatedByUserID: "u_admin",
	})
	require.NoError(t, err)

	hostsService := do.MustInvoke[services.Hosts](backend.Injector())
	resp, err := hostsService.RegisterAgentHost(ctx, &agentpb.RegisterHostRequest{
		RegistrationToken: tokenPlain,
	...
	rows, err := queries.ListAuditLogs(ctx, db.ListAuditLogsParams{
		Limit:  100,
		Offset: 0,
	})
	require.NoError(t, err)

	var matched *db.AuditLog
	for i := range rows {
		if rows[i].Action == "host.create" && rows[i].TargetID.UnwrapOr("") == resp.HostId {
			matched = &rows[i]
			break
		}
	}
	require.NotNil(t, matched, "expected a host.create audit entry for the agent-registered host")
	assert.Equal(t, "u_admin", matched.ActorID.UnwrapOr(""))
	assert.Equal(t, "admin@patchbase.local", matched.ActorEmail)
	assert.Equal(t, "203.0.113.42", matched.IpAddress.UnwrapOr(""))
}
